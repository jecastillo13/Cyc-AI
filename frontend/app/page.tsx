  ...
  useEffect(()=>{
    ...
    if(new URLSearchParams(window.location.search).get("connected")==="strava"){
      fetch("/api/integrations/strava/sync",{method:"POST"})
        .then(async response=>{const payload=await response.json();if(!response.ok)throw new Error(payload.error);setData(await fetchDashboard());setConnected(true);setMessage(`${payload.synced} actividades sincronizadas; ${payload.streams_analyzed||0} analizadas en detalle.`)})
