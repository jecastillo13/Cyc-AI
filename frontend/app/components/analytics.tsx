...
function AnalyticsContent({data}:{data:Analytics}){const maxTss=Math.max(1,...data.weekly.map(item=>item.tss));return <>
 <div className="analytics-kpis"><Kpi label="Actividades" value={String(data.totals.activities)}/><Kpi label="Duración" value={`${data.totals.duration_hours} h`}/><Kpi label="Distancia" value={`${data.totals.distance_km} km`}/><Kpi label="TSS total" value={String(data.totals.tss)}/><Kpi label="Potencia máxima" value={`${data.peaks.power||"—"} W`}/><Kpi label="Pulso máximo" value={`${data.peaks.heart_rate||"—"} bpm`}/></div>
 <div className="analytics-grid"><article><h3>Carga semanal (TSS)</h3><div className="weekly-bars">{data.weekly.map(item=><div key={item.week}><span style={{height:`${Math.max(3,item.tss/maxTss*100)}%`}}/><small>{item.week.slice(5)}<br/>{item.tss}</small></div>)}</div></article><article><h3>Resumen por deporte</h3>{data.sports.map(item=><div className="sport-row" key={item.sport}><strong>{item.sport}</strong><span>{item.activities} sesiones</span><span>{item.duration_hours} h</span><span>{item.distance_km} km</span></div>)}</article></div>
 ...
 <p className="analytics-note">Las curvas se calculan desde streams de Strava. La carga se estima con duración hasta que Cyc-AI calcule un modelo propio basado en potencia. Las zonas son estimadas hasta que configures tu FTP y frecuencia cardíaca máxima.</p>
 </>}
