...
export function ZoneSettings(){
 const [values,setValues]=useState<Draft>(defaults),[message,setMessage]=useState(""),[saving,setSaving]=useState(false),[openHelp,setOpenHelp]=useState<Key|null>(null);
 ...
 const field=(key:Key,label:string,unit:string,placeholder:string)=><label><span className="field-title">{label}<button className="help-button" type="button" aria-expanded={openHelp===key} aria-label={`Ayuda sobre ${label}`} onClick={()=>setOpenHelp(current=>current===key?null:key)}>?</button></span><span className="field-input"><input type="text" inputMode="numeric" pattern="[0-9]*" placeholder={placeholder} value={values[key]} onChange={event=>setValues(current=>({...current,[key]:event.target.value.replace(/\D/g,"")}))}/>{unit}</span>{openHelp===key&&<span className="field-help" role="status">{help[key]}</span>}</label>;
