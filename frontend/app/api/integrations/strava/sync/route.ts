  ...
  try {
    ...
    for (const item of rows) {
      ...
      if(!analyzed.get(String(item.id))&&streamsAnalyzed<20){
        ...
        if(streamResponse.ok){const parseLimits=(value:string|null)=>{try{return JSON.parse(value||"[]") as number[]}catch{return []}};const analysis=analyzeStravaStreams(await streamResponse.json() as never,{powerLimits:parseLimits(user.powerZoneLimits),heartRateLimits:parseLimits(user.heartRateZoneLimits),ftpWatts:user.ftpWatts||0});await db.update(activities).set({powerCurve:JSON.stringify(analysis.powerCurve),normalizedPower:analysis.normalizedPower||null,trainingStressScore:analysis.estimatedTss?Math.round(analysis.estimatedTss*10):null,intensityFactor:analysis.intensityFactor?Math.round(analysis.intensityFactor*1000):null,maxPower:analysis.maxPower||null,maxHeartRate:analysis.maxHeartRate||null,elevationMeters:analysis.elevationMeters||null,powerZones:JSON.stringify(analysis.powerZones),heartRateZones:JSON.stringify(analysis.heartRateZones),streamAnalyzedAt:now}).where(and(eq(activities.userId,user.userId),eq(activities.provider,"strava"),eq(activities.externalId,String(item.id))));streamsAnalyzed++}
      }
    }
