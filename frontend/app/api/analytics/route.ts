...
export async function GET(request:Request){
  try{
    ...
    const trainingPeaks=raw.filter(row=>row.provider==="trainingpeaks"&& (row.durationSeconds>0||row.distanceMeters>0||row.trainingStressScore));
    const rows=trainingPeaks.length?trainingPeaks:raw.filter(row=>row.durationSeconds>0||row.distanceMeters>0);
